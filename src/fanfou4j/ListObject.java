 ...
 */
public class ListObject extends FanfouResponse implements java.io.Serializable {
	...
	/*package*/ListObject(Response res, Fanfou fanfou) throws FanfouException {
		super(res);
		init(res, res.asDocument().getDocumentElement(), fanfou);
	}

	/*package*/ListObject(Response res, Element elem, Fanfou fanfou) throws FanfouException {
		super(res);
		init(res, elem, fanfou);
	}

	/*package*/ListObject(JSONObject json) throws FanfouException {
		try {
			id = json.getLong("id");
	...
	private void init(Response res, Element elem, Fanfou fanfou) throws FanfouException {
		...
		if (statuses.getLength() != 0) {
			...
			user = new User(res, userElem, fanfou);
		}
	}
	 ...
	 * @param res		Fanfou4j http请求返回对象
	 * @param fanfou		Weibo对象实例
	 * @return		如果出现错误，或者结果为空，则返回空的ListObjectWapper对象
	 * @throws FanfouException
	 */
	/*package*/static ListObjectWapper constructListObjects(Response res, Fanfou fanfou) throws FanfouException {
		...
		if (isRootNodeNilClasses(doc)) {
			...
			try {
				...
				for (int i = 0; i < length; i++) {
					...
					lists.add(new ListObject(res, status, fanfou));
				}
				//
	 ...
	 * @throws FanfouException
	 */
	/*package*/static ListObjectWapper constructListObjects(Response res) throws FanfouException {
